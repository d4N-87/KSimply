<script lang="ts">
	...
	async function analyzeHardware(event: SubmitEvent) {
		...
		progressPercent = 10;
		progressText = m.analyze_button_loading();
		...
		{:else if analysisResults.length > 0}
			<div class="hud-panel mb-8 p-4 bg-surface/50 border border-border rounded-lg space-y-4">
				...
				{#if modelTypes.length > 1}
					<div class="flex items-center gap-4 flex-wrap">
						<span class="font-semibold text-secondary-text min-w-max">Tipologia:</span>
						<div class="flex gap-2 flex-wrap">
							{#each modelTypes as type (type)}
								<button
									onclick={() => toggleTypeFilter(type)}
									class="{baseClass} {activeTypeFilters.has(type)
										? 'bg-primary-accent text-background font-bold ring-2 ring-primary-accent/50'
										: 'bg-surface hover:bg-border'}"
								>
									{type}
								</button>
			...
			<h2 class="text-3xl font-bold text-center mb-8">
				{m.results_title({ count: filteredResults.length })}
			</h2>
