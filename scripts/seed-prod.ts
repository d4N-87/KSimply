...
async function seed() {
	...
	for (const release of model_releases) { if (release && release.model_name) { await db.execute({ sql: 'INSERT INTO model_releases (model_id, quantization_id, file_size_gb, repository) VALUES (?, ?, ?, ?)', args: [nameToIdMaps['base_models'].get(release.model_name), nameToIdMaps['quantizations'].get(release.quantization_name), release.file_size_gb, release.repository] }); } }
	console.log(`   -> Inserite ${model_releases.length} versioni di modelli.`);
	for (const release of text_encoder_releases) { if (release && release.encoder_name) { await db.execute({ sql: 'INSERT INTO text_encoder_releases (encoder_id, quantization_id, file_size_gb, repository) VALUES (?, ?, ?, ?)', args: [nameToIdMaps['text_encoders'].get(release.encoder_name), nameToIdMaps['quantizations'].get(release.quantization_name), release.file_size_gb, release.repository] }); } }
	console.log(`   -> Inserite ${text_encoder_releases.length} versioni di text encoder.`);
	for (const release of vae_releases) { if (release && release.vae_name) { await db.execute({ sql: 'INSERT INTO vae_releases (vae_id, quantization_id, file_size_gb, repository) VALUES (?, ?, ?, ?)', args: [nameToIdMaps['vaes'].get(release.vae_name), nameToIdMaps['quantizations'].get(release.quantization_name), release.file_size_gb, release.repository] }); } }
